...
> 
> **Réponse :** base64 ≠ sécurité. Le chiffrement etcd via Cloud KMS est la seule garantie que tes mots de passe, tokens et certificats stockés dans les Secrets Kubernetes sont protégés même en cas de compromission de l'infrastructure sous-jacente.
...
> 
> **Réponse :** **Charge :** 80 events/s ÷ **20 events/s par pod** = **4 réplicas** 
> Chaque pod tourne à **60% CPU** (exactement la cible HPA), la charge est parfaitement absorbée et on reste dans les bornes `min:2 / max:10`.
...
> 
> **Réponse :** Avec 6 partitions et 3 instances dans le même Consumer Group (`tracker-service-group`), Kafka effectue un **rebalancing** et attribue **2 partitions par instance** (6 ÷ 3 = 2). Chaque partition n'est lue que par une seule instance à la fois, garantissant l'ordre des messages par camion (la clé de partition étant le `truck_id`). Si on ajoute une **4e instance**, celle-ci reste **idle** (aucune partition assignée) car le nombre d'instances dépasse le nombre de partitions — Kafka ne peut pas diviser une partition entre plusieurs consumers d'un même groupe. Pour exploiter une 4e instance, il faudrait d'abord augmenter le nombre de partitions du topic (ex : passer à 8 ou 12).
...
|RAM moyenne tracker-consumer|~85 Mi|
...
> 
> **Réponse :**
> 1. **Augmenter le nombre de réplicas du tracker-consumer** (scale horizontal) : passer de 3 à 6 replicas. Avec 6 partitions sur le topic `truck-positions`, chaque instance traite 1 partition au lieu de 2, ce qui divise la charge par deux immédiatement. C'est l'action la plus rapide et sans impact sur le code.
> 2. **Optimiser le traitement par message** : augmenter le paramètre `partitionsConsumedConcurrently`, traiter les messages en batch (`eachBatch` plutôt que `eachMessage`), et supprimer les éventuels appels bloquants synchrones dans le handler (ex : appels DB non-awaités correctement). Cela améliore le débit sans changer l'infrastructure.
> 3. **Augmenter le nombre de partitions du topic** `truck-positions` (ex : passer de 6 à 12) pour permettre un parallelisme supérieur au-delà de 6 consumers. Attention : on ne peut qu'**augmenter** le nombre de partitions en Kafka, jamais le réduire, et cela redistribue les clés ce qui peut casser temporairement l'ordre par `truck_id`.
